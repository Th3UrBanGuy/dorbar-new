  ...
  return (
    <div className="fixed bottom-6 right-6 z-[999] flex flex-col items-end gap-3 pointer-events-none">
      ...
      <AnimatePresence>
        {isExpanded && (
          <motion.div
            ...
            className="bg-white/95 backdrop-blur-3xl p-3 sm:p-4 rounded-[2rem] border border-white shadow-[0_20px_40px_-5px_rgba(0,0,0,0.15),inset_0_2px_4px_rgba(255,255,255,0.8)] pointer-events-auto flex flex-col items-center w-[280px] sm:w-[320px]"
          >
            <div className="flex items-center justify-between w-full px-2 mb-3">
              <div className="flex items-center gap-2">
                 <Languages className="w-4 h-4 text-emerald-600" />
                 <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-none mt-0.5">Language</span>
              </div>
              <button 
                onClick={() => setIsExpanded(false)}
                className="w-7 h-7 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 hover:bg-slate-200 hover:text-slate-600 transition-colors"
                title="Close"
              >
                ...
                return (
                  ...
                  >
                    <div className={`
                      ...
                      ${isSelected 
                        ? 'from-emerald-400 to-emerald-500 shadow-[inset_0_2px_4px_rgba(255,255,255,0.4),0_4px_12px_rgba(16,185,129,0.3)] ring-2 ring-emerald-500 ring-offset-2' 
                        : 'from-slate-50 to-slate-100 shadow-[0_4px_10px_rgba(0,0,0,0.05),inset_0_2px_4px_rgba(255,255,255,1)] hover:-translate-y-1 hover:shadow-md'
                      }
                    `}>
                      {isSelected && <Sparkles className="absolute -top-1 -right-1 w-3 h-3 text-emerald-300 z-10 animate-pulse" />}
                      <span className="relative z-0 drop-shadow-md">{lang.flag}</span>
                    </div>
                    <span className={`text-[8px] sm:text-[9px] font-bold uppercase tracking-wider transition-colors ${isSelected ? 'text-emerald-600' : 'text-slate-500 group-hover:text-slate-800'}`}>
                      {lang.code}
                    </span>
                  </button>
                );
            ...
            <AnimatePresence>
              {isInjecting && (
                ...
                >
                   <div className="flex bg-white shadow-xl px-5 py-2.5 rounded-full items-center gap-3 border border-slate-100">
      ...
      <motion.button 
        ...
        className="w-14 h-14 rounded-full bg-gradient-to-b from-white to-slate-50 shadow-[0_8px_20px_rgba(0,0,0,0.1),inset_0_2px_4px_rgba(255,255,255,1)] border border-slate-200/60 flex items-center justify-center pointer-events-auto group hover:scale-105 active:scale-95 transition-all relative z-50 backdrop-blur-3xl ring-4 ring-white/50"
        whileTap={{ scale: 0.9 }}
        title="Translate Website"
      >
        <Globe2 className="w-6 h-6 text-emerald-600 drop-shadow-sm group-hover:rotate-12 transition-transform duration-500" />
