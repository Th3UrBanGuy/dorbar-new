...
import { BookOpen, Heart, LogOut, ChevronLeft, Search, Bell, Activity, BookHeart } from "lucide-react";

export default function HadithDashboardLoading() {
  return (
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-white md:rounded-l-[3rem] shadow-[-10px_0_30px_rgba(0,0,0,0.02)] relative">
        ...
        <div className="flex-1 overflow-y-auto px-6 sm:px-10 pb-24 md:pb-10 flex flex-col xl:flex-row gap-10">
          ...
          <div className="flex-1 flex flex-col gap-8 order-2 xl:order-1 pt-6 sm:pt-0">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 2xl:grid-cols-3 gap-5">
               {Array.from({ length: 9 }).map((_, i) => (
                  <div key={i} className="bg-white rounded-[2.5rem] p-6 shadow-sm border border-slate-100 flex flex-col h-48 animate-pulse">
                     <div className="w-16 h-16 bg-slate-50 rounded-3xl mb-6"></div>
                     <div className="w-3/4 h-6 bg-slate-100 rounded-lg mt-auto"></div>
                  </div>
               ))}
             </div>
          </div>
          ...
          <aside className="w-full xl:w-80 flex flex-col gap-8 shrink-0 order-1 xl:order-2 pb-10 xl:pt-14 animate-pulse">
             ...
             <div className="bg-emerald-50/10 h-64 rounded-3xl border border-emerald-50"></div>
          </aside>
